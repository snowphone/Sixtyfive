...
class Sixtyfive(private val configName: String = "configs.json") {
    ...
    private val accessToken = "Jgl5ZIYICzsAAAAAAAAAAdzXB7cP5CdmfMlPZ0dBnmMa0L2mNKWPvVqMLkzBPCaq" //TODO("Get access token from the web")
    ...
    fun restore(processName: String) {
        val stream = ByteArrayOutputStream()
            .also { dropbox.files().downloadBuilder("/data/${processName.toZipName}").download(it) }
            .toByteArray()
            .inputStream()
    ...
    fun backup(processName: String) {
        ...
        dropbox.files()
            .uploadBuilder("/data/${processName.toZipName}")
